...
\title[MicromanEdge: a monitoring and orchestration protocol for microservices in Edge Environments]{MicromanEdge: a monitoring and orchestration protocol for microservices in edge environments}

% Autor
% use [f] for female and [m] for male
\authorname[m]{Nuno Morais}{Nuno Morais}
...
% use [f] for female and [m] for male
\adviser[m]{João Leitão}{Assistant Professor}{\\& NOVA University of Lisbon} 
...
\chapterfile{proposed_solution}
\chapterfile{planning}


% Text appendixes
% sintax: \appendixfile{file}
% Comment if not needed
\appendixfile{appendix/appendix1}
\annexfile{annex/annex1}

% BibTeX bibiography files. Can be used multiple times with a single file name each time.
